/**
 * RIASEC Engine — Holland Vocational Model
 * Calculates user profile vector and matches against career vectors.
 */
 ...
 * @param {Array} answers - [{questionId, answer, category, weight}]
 * @returns {Object} - {R, I, A, S, E, C} scores normalized 0-100
 */
function calculateScores(answers) {
  ...
  for (const a of answers) {
    ...
    maxPossible[cat] += 5 * weight; // Max Likert = 5
  }

  // Normalize to 0-100
  const scores = {};
  for (const cat of CATEGORIES) {
    scores[cat] = maxPossible[cat] > 0 ? Math.round((raw[cat] / maxPossible[cat]) * 100) : 0;
  }
...
/**
 * Generate profile label from top 2 scores
 * @param {Object} scores - {R, I, A, S, E, C}
 * @returns {string} - e.g. "Investigador-Emprendedor"
 */
function generateProfileLabel(scores) {
  ...
  const top1 = CATEGORY_LABELS[sorted[0].cat];
  const top2 = CATEGORY_LABELS[sorted[1].cat];
  return `${top1}-${top2}`;
}
...
/**
 * Match user scores against career RIASEC vectors
 * @param {Object} userScores - {R, I, A, S, E, C}
 * @param {Array} careerVectors - [{career_id, r, i, a, s, e, c, ...careerInfo}]
 * @returns {Array} - Top careers sorted by similarity
 */
...
module.exports = {
  CATEGORIES, CATEGORY_LABELS, CATEGORY_DESCRIPTIONS,
  calculateScores, generateProfileLabel, matchCareers, cosineSimilarity,
};
