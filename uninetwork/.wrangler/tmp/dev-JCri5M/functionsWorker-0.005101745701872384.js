...
// .wrangler/tmp/bundle-qtP1yw/strip-cf-connecting-ip-header.js
...
var resolveCallback = /* @__PURE__ */ __name2(async (str, phase, preserveCallbacks, context22, buffer) => {
...
var JWT_EXPIRES_SECONDS = 7 * 24 * 60 * 60;
function b64url(buf) {
...
var BOLIVIAN_DEPARTMENTS = ["LP", "CB", "SC", "OR", "PT", "TJ", "CH", "BE", "PA"];
...
var db = {
  get: async (env22, sql, ...p) => {
    const rs = await env22.DB.prepare(sql).bind(...p.flat()).first();
    return rs || null;
  },
  all: async (env22, sql, ...p) => {
    const rs = await env22.DB.prepare(sql).bind(...p.flat()).all();
    return rs.results || [];
  },
  run: async (env22, sql, ...p) => {
    const rs = await env22.DB.prepare(sql).bind(...p.flat()).run();
...
app.post("/api/posts/:postId/like", auth, async (c) => {
  ...
  if (existing) {
    await db.run(c.env, "DELETE FROM post_likes WHERE user_id=? AND post_id=?", userId, postId);
  } else {
    await db.run(c.env, "INSERT INTO post_likes (user_id,post_id) VALUES (?,?)", userId, postId);
  }
...
app.post("/api/vocational/submit", auth, async (c) => {
  ...
  await db.run(c.env, `INSERT INTO vocational_results (user_id,r_score,i_score,a_score,s_score,e_score,c_score,profile_label,recommended_careers) VALUES (?,?,?,?,?,?,?,?,?)`, c.get("user").id, scores.R, scores.I, scores.A, scores.S, scores.E, scores.C, profileLabel, JSON.stringify(top5.map((c2) => c2.career_id)));
...
// .wrangler/tmp/bundle-qtP1yw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qtP1yw/middleware-loader.entry.ts
