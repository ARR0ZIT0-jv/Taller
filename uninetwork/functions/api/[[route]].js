...
// ── D1 helpers ───────────────────────────────────────────────────────────────
const db = {
  get: (DB, sql, ...p) => DB.prepare(sql).bind(...p.flat()).first(),
  all: async (DB, sql, ...p) => { const r = await DB.prepare(sql).bind(...p.flat()).all(); return r.results; },
  run: (DB, sql, ...p) => DB.prepare(sql).bind(...p.flat()).run(),
};
